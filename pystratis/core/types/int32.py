...
class int32(customint):
    ...
    def __radd__(self, other) -> int32:
        if other == 0:
            return self
        else:
            return self.__add__(other)

    def __sub__(self, other) -> int32:
        if isinstance(other, customint):
            return int32(self.value - other.value)
        if isinstance(other, int):
            return int32(self.value - other)
    ...
    def __rsub__(self, other) -> int32:
        if other == 0:
            return self
        else:
            return self.__sub__(other)

    def __mul__(self, other) -> int32:
        if isinstance(other, customint):
            return int32(self.value * other.value)
        if isinstance(other, int):
            return int32(self.value * other)
    ...
    def __imul__(self, other) -> int32:
        if other == 0:
            return int32(0)
        else:
            return self.__mul__(other)

    def __rmul__(self, other) -> int32:
        if other == 0:
            return int32(0)
        else:
            return self.__mul__(other)

    def __divmod__(self, other) -> Tuple[int32, int32]:
        if isinstance(other, customint):
            if other.value == 0:
                raise ZeroDivisionError('Cannot divide by zero.')
            new_quot, new_mod = self.value // other.value, self.value % other.value
            return int32(new_quot), int32(new_mod)
        if isinstance(other, int):
            if other == 0:
                raise ZeroDivisionError('Cannot divide by zero.')
    ...
    def __rdivmod__(self, other) -> Tuple[int32, int32]:
        return self.__divmod__(other)

    def __floordiv__(self, other) -> int32:
        if isinstance(other, customint):
            if other.value == 0:
                raise ZeroDivisionError('Cannot divide by zero.')
            return int32(self.value // other.value)
    ...
    def __pow__(self, power, modulo=None) -> int32:
        if isinstance(power, int):
            return int32(self.value ** power)
