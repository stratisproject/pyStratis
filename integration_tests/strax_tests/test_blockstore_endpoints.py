...
def test_addressindexer_tip(hot_node: BaseNode):
    response = hot_node.blockstore.addressindexer_tip()
    assert isinstance(response, AddressIndexerTipModel)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_block(hot_node: BaseNode):
    ...
    response = hot_node.blockstore.block(request_model=request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_get_block_count(hot_node: BaseNode):
    response = hot_node.blockstore.get_block_count()
    assert isinstance(response, int)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_get_addresses_balances(hot_node: BaseNode, syncing_node: BaseNode,
    ...
    response = hot_node.blockstore.get_addresses_balances(request_model=request_model)
    ...
    hot_node.blockstore.get_addresses_balances(request_model=request_model)
    assert isinstance(response, GetAddressesBalancesModel)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_get_verbose_addresses_balances(hot_node: BaseNode, syncing_node: BaseNode,
                                        get_node_address_with_balance: Callable):
    mining_address = get_node_address_with_balance(hot_node)
    receiving_address = get_node_address_with_balance(syncing_node)
    addresses = [mining_address, receiving_address]
    request_model = GetVerboseAddressesBalancesRequest(addresses=addresses[0])
    response = hot_node.blockstore.get_verbose_addresses_balances(request_model=request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_get_utxoset(hot_node: BaseNode):
    height = hot_node.blockstore.get_block_count()
    request_model = GetUTXOSetRequest(at_block_height=height)
    response = hot_node.blockstore.get_utxo_set(request_model=request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_get_last_balance_update_transaction(hot_node: BaseNode, syncing_node: BaseNode,
                                             get_node_address_with_balance: Callable):
    receiving_address = get_node_address_with_balance(syncing_node)
    request_model = GetLastBalanceUpdateTransactionRequest(address=receiving_address)
    response = hot_node.blockstore.get_last_balance_update_transaction(request_model=request_model)
