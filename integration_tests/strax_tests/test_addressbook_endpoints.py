...
def test_add_address(strax_hot_node: BaseNode, random_addresses):
    ...
    for i in range(len(addresses)):
        response = strax_hot_node.address_book.add(address=addresses[i], label=f'Label{i}')
...
def test_remove_address(strax_hot_node: BaseNode, generate_p2pkh_address):
    ...
    strax_hot_node.address_book.add(address=address, label='AddressToRemove')

    response = strax_hot_node.address_book.remove(label='AddressToRemove')
    assert isinstance(response, AddressBookEntryModel)
...
def test_get_address_book(strax_hot_node: BaseNode, random_addresses):
    addressbookentries = strax_hot_node.address_book()
    if len(addressbookentries) < 2:
        addresses = random_addresses(network=strax_hot_node.blockchainnetwork)
        for i in range(len(addresses)):
            strax_hot_node.address_book.add(address=addresses[i], label=f'NewLabel{i}')

    response = strax_hot_node.address_book(skip=1, take=1)
