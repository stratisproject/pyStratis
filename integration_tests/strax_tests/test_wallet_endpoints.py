...
def test_mnemonic(hot_node: BaseNode):
    request_model = MnemonicRequest(language='English', word_count=12)
    response = hot_node.wallet.mnemonic(request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_create(hot_node: BaseNode):
    request_model = MnemonicRequest(language='English', word_count=12)
    mnemonic = hot_node.wallet.mnemonic(request_model)
    ...
    response = hot_node.wallet.create(request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_sign_message(hot_node: BaseNode, get_node_address_with_balance):
    message = 'The Times 03/Jan/2009 Chancellor on brink of second bailout for banks.'
    address = get_node_address_with_balance(hot_node)
    ...
    response = hot_node.wallet.sign_message(request_model)
    assert isinstance(response, str)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_pubkey(hot_node: BaseNode, get_node_address_with_balance):
    address = get_node_address_with_balance(hot_node)
    request_model = PubKeyRequest(wallet_name='Test', external_address=address)
    response = hot_node.wallet.pubkey(request_model)
    assert isinstance(response, PubKey)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_verify_message(hot_node: BaseNode, get_node_address_with_balance):
    message = 'The Times 03/Jan/2009 Chancellor on brink of second bailout for banks.'
    address = get_node_address_with_balance(hot_node)
    request_model = SignMessageRequest(
        wallet_name='Test',
    ...
    signature = hot_node.wallet.sign_message(request_model)
    request_model = VerifyMessageRequest(
        signature=signature,
        external_address=address,
        message=message
    )
    assert hot_node.wallet.verify_message(request_model)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_load(hot_node: BaseNode):
    request_model = LoadRequest(name='Test', password='password')
    hot_node.wallet.load(request_model)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_recover(hot_node: BaseNode, get_datetime):
    request_model = MnemonicRequest(language='English', word_count=12)
    mnemonic = hot_node.wallet.mnemonic(request_model)
    mnemonic = ' '.join(mnemonic)
    ...
    hot_node.wallet.recover(request_model)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_recover_via_extpubkey(hot_node: BaseNode, get_datetime):
    request_model = ExtPubKeyRequest(wallet_name='Test', account_name='account 0')
    extpubkey = hot_node.wallet.extpubkey(request_model)
    ...
    hot_node.wallet.recover_via_extpubkey(request_model)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_general_info(hot_node: BaseNode):
    request_model = GeneralInfoRequest(name='Test')
    response = hot_node.wallet.general_info(request_model)
    assert isinstance(response, WalletGeneralInfoModel)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_transaction_count(hot_node: BaseNode):
    request_model = AccountRequest(wallet_name='Test', account_name='account 0')
    response = hot_node.wallet.transaction_count(request_model)
    assert isinstance(response, int)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_history(hot_node: BaseNode, get_node_address_with_balance):
    address = get_node_address_with_balance(hot_node)
    ...
    response = hot_node.wallet.history(request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_balance(hot_node: BaseNode):
    request_model = BalanceRequest(
        wallet_name='Test',
        account_name='account 0',
        include_balance_by_address=True
    )
    response = hot_node.wallet.balance(request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_received_by_address(hot_node: BaseNode, get_node_address_with_balance):
    address = get_node_address_with_balance(hot_node)
    request_model = ReceivedByAddressRequest(address=address)
    response = hot_node.wallet.received_by_address(request_model)
    assert isinstance(response, AddressBalanceModel)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_max_balance(hot_node: BaseNode):
    ...
    response = hot_node.wallet.max_balance(request_model)
    assert isinstance(response, MaxSpendableAmountModel)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_spendable_transactions(hot_node: BaseNode):
    request_model = SpendableTransactionsRequest(wallet_name='Test', account_name='account 0', min_confirmations=0)
    response = hot_node.wallet.spendable_transactions(request_model)
...
def test_estimate_txfee(
        hot_node: BaseNode,
        syncing_node: BaseNode,
        get_node_unused_address,
        get_node_address_with_balance):
    destination_address = get_node_unused_address(syncing_node)
    change_address = get_node_address_with_balance(hot_node)
    ...
    response = hot_node.wallet.estimate_txfee(request_model)
...
def test_build_transaction(
    ...
    response = hot_node.wallet.build_transaction(request_model)
...
def test_build_interflux_transaction(
    ...
    response = hot_node.wallet.build_interflux_transaction(request_model)
    assert isinstance(response, BuildTransactionModel)
    assert isinstance(response.transaction_id, uint256)
    assert isinstance(response.hex, hexstr)
    assert isinstance(response.fee, Money)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_send_transaction(hot_node: BaseNode, syncing_node: BaseNode,
    ...
    built_transaction = hot_node.wallet.build_transaction(request_model)

    request_model = SendTransactionRequest(hex=built_transaction.hex)
    response = hot_node.wallet.send_transaction(request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_list_wallets(hot_node: BaseNode):
    response = hot_node.wallet.list_wallets()
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_account(hot_node: BaseNode):
    request_model = GetUnusedAccountRequest(password='password', wallet_name='Test')
    response = hot_node.wallet.account(request_model)
    assert isinstance(response, str)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_accounts(hot_node: BaseNode):
    request_model = GetAccountsRequest(wallet_name='Test')
    response = hot_node.wallet.accounts(request_model)
    assert isinstance(response, list)
    for item in response:
        assert isinstance(item, str)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_unused_address(hot_node: BaseNode):
    request_model = GetUnusedAddressRequest(wallet_name='Test', account_name='account 0', segwit=False)
    response = hot_node.wallet.unused_address(request_model)
    assert isinstance(response, Address)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_unused_addresses(hot_node: BaseNode):
    ...
    response = hot_node.wallet.unused_addresses(request_model)
    assert isinstance(response, list)
    for item in response:
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_new_addresses(hot_node: BaseNode):
    ...
    response = hot_node.wallet.new_addresses(request_model)
    assert isinstance(response, list)
    for item in response:
        assert isinstance(item, Address)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_remove_transactions(hot_node: BaseNode, get_datetime):
    from api.wallet.requestmodels import SpendableTransactionsRequest
    request_model = SpendableTransactionsRequest(wallet_name='Test', account_name='account 0', min_confirmations=10)
    spendable_transactions = hot_node.wallet.spendable_transactions(request_model)
    ...
    response = hot_node.wallet.remove_transactions(request_model)
    assert isinstance(response, list)
    for item in response:
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_remove_wallet(hot_node: BaseNode):
    request_model = MnemonicRequest(language='English', word_count=12)
    mnemonic = hot_node.wallet.mnemonic(request_model)
    mnemonic = ' '.join(mnemonic)
    request_model = CreateRequest(
    ...
    hot_node.wallet.create(request_model)
    request_model = RemoveWalletRequest(wallet_name='TestRemove')
    hot_node.wallet.remove_wallet(request_model)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_extpubkey(hot_node: BaseNode):
    request_model = ExtPubKeyRequest(wallet_name='Test', account_name='account 0')
    response = hot_node.wallet.extpubkey(request_model)
    assert isinstance(response, ExtPubKey)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_private_key(hot_node: BaseNode, get_node_address_with_balance):
    address = get_node_address_with_balance(hot_node)
    request_model = PrivateKeyRequest(password='password', wallet_name='Test', address=address)
    response = hot_node.wallet.private_key(request_model)
    assert isinstance(response, str)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_sync(hot_node: BaseNode):
    from api.consensus.requestmodels import GetBlockHashRequest
    request_model = GetBlockHashRequest(height=1)
    block_hash = hot_node.consensus.get_blockhash(request_model)
    request_model = SyncRequest(hash=block_hash)
    hot_node.wallet.sync(request_model)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_sync_from_date(hot_node: BaseNode, get_datetime):
    request_model = SyncFromDateRequest(date=get_datetime(365), all=True, wallet_name='Test')
    hot_node.wallet.sync_from_date(request_model)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_wallet_stats(hot_node: BaseNode):
    ...
    response = hot_node.wallet.wallet_stats(request_model)
    assert isinstance(response, WalletStatsModel)


@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_split_coins(hot_node: BaseNode, node_mines_some_blocks_and_syncs):
    node_mines_some_blocks_and_syncs(mining_node=hot_node, num_blocks_to_mine=15)
    ...
    response = hot_node.wallet.split_coins(request_model)
    assert isinstance(response, WalletSendTransactionModel)
    assert isinstance(response.transaction_id, uint256)
    for item in response.outputs:
        assert isinstance(item, TransactionOutputModel)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_distribute_utxos(hot_node: BaseNode, node_mines_some_blocks_and_syncs):
    node_mines_some_blocks_and_syncs(mining_node=hot_node, num_blocks_to_mine=15)
    from api.wallet.requestmodels import SpendableTransactionsRequest
    request_model = SpendableTransactionsRequest(wallet_name='Test', account_name='account 0', min_confirmations=10)
    spendable_transactions = hot_node.wallet.spendable_transactions(request_model)
    ...
    response = hot_node.wallet.distribute_utxos(request_model)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_sweep(hot_node: BaseNode, get_node_address_with_balance):
    address = get_node_address_with_balance(hot_node)
    ...
    private_key = hot_node.wallet.private_key(request_model)
    request_model = SweepRequest(
        private_keys=[private_key],
        destination_address=address,
        broadcast=False
    )
    response = hot_node.wallet.sweep(request_model)
    assert isinstance(response, list)
    for item in response:
...
def test_build_offline_sign_request(
    ...
    response = hot_node.wallet.build_offline_sign_request(request_model)
...
def test_offline_sign_request(
    ...
    offline_sign_model = hot_node.wallet.build_offline_sign_request(request_model)
    ...
    response = hot_node.wallet.offline_sign_request(request_model)
    assert isinstance(response, BuildTransactionModel)
    assert isinstance(response.transaction_id, uint256)
    assert isinstance(response.hex, hexstr)
    assert isinstance(response.fee, Money)
...
@pytest.mark.integration_test
@pytest.mark.strax_integration_test
def test_consolidate(hot_node: BaseNode, get_node_address_with_balance, node_mines_some_blocks_and_syncs):
    node_mines_some_blocks_and_syncs(mining_node=hot_node, num_blocks_to_mine=15)
    from api.wallet.requestmodels import SpendableTransactionsRequest
    request_model = SpendableTransactionsRequest(wallet_name='Test', account_name='account 0', min_confirmations=10)
    spendable_transactions = hot_node.wallet.spendable_transactions(request_model)
    ...
    destination_address = get_node_address_with_balance(hot_node)
    ...
    response = hot_node.wallet.consolidate(request_model)
