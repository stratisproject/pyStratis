...
def test_account_addresses(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode):
    ...
    request_model = AccountAddressesRequest(wallet_name='Test')
    response = cirrus_syncing_node.smart_contract_wallet.account_addresses(request_model)
    assert isinstance(response, list)
    for item in response:
        assert isinstance(item, Address)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_address_balance(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode):
    ...
    response = cirrus_syncing_node.smart_contract_wallet.address_balance(request_model)
    assert isinstance(response, Money)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_history(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode):
    ...
    response = cirrus_syncing_node.smart_contract_wallet.history(request_model)
    assert isinstance(response, list)
    for item in response:
        assert isinstance(item, ContractTransactionItemModel)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_create(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode):
    ...
    response = cirrus_syncing_node.smart_contract_wallet.create(request_model)
    assert isinstance(response, uint256)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_call(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode):
    ...
    response = cirrus_syncing_node.smart_contract_wallet.call(request_model)
    assert isinstance(response, BuildContractTransactionModel)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_send_transaction(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode):
    ...
    response = cirrus_syncing_node.smart_contract_wallet.send_transaction(request_model)
    assert isinstance(response, WalletSendTransactionModel)
