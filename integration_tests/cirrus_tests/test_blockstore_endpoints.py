...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_block(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode, wait_x_blocks_and_sync):
    ...
    # Request json output with maximum details.
    block_hash = cirrus_syncing_node.consensus.get_best_blockhash()
    request_model = BlockRequest(hash=block_hash, show_transaction_details=True, output_json=True)
    response = cirrus_syncing_node.blockstore.block(request_model=request_model)
    assert isinstance(response, BlockModel)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_get_block_count(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode, wait_x_blocks_and_sync):
    wait_x_blocks_and_sync(1)
    response = cirrusminer_node.blockstore.get_block_count()
    assert isinstance(response, int)

    response = cirrus_syncing_node.blockstore.get_block_count()
    assert isinstance(response, int)
...
def test_get_addresses_balances(cirrusminer_node: CirrusMinerNode,
    ...
    receiving_address = get_node_address_with_balance(cirrus_syncing_node)
    ...
    response = cirrus_syncing_node.blockstore.get_addresses_balances(request_model=request_model)
    assert isinstance(response, GetAddressesBalancesModel)
    assert len(response.balances) == 1
    assert isinstance(response.balances[0], AddressBalanceModel)
    ...
    cirrus_syncing_node.blockstore.get_addresses_balances(request_model=request_model)
    assert isinstance(response, GetAddressesBalancesModel)
    for item in response.balances:
        assert isinstance(item, AddressBalanceModel)
...
def test_get_verbose_addresses_balances(cirrusminer_node: CirrusMinerNode,
    ...
    response = cirrus_syncing_node.blockstore.get_verbose_addresses_balances(request_model=request_model)
    assert isinstance(response, GetVerboseAddressesBalancesModel)
    assert len(response.balances_data) == 1
    assert isinstance(response.balances_data[0], VerboseAddressBalanceModel)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_get_utxoset(cirrusminer_node: CirrusMinerNode, cirrus_syncing_node: CirrusNode, wait_x_blocks_and_sync):
    wait_x_blocks_and_sync(1)
    ...
    response = cirrus_syncing_node.blockstore.get_utxo_set(request_model=request_model)
    assert isinstance(response, list)
    for item in response:
        assert isinstance(item, UTXOModel)
...
@pytest.mark.integration_test
@pytest.mark.cirrus_integration_test
def test_get_last_balance_update_transaction(cirrusminer_node: CirrusMinerNode,
                                             cirrus_syncing_node: CirrusNode,
                                             wait_x_blocks_and_sync,
                                             get_node_address_with_balance):
    wait_x_blocks_and_sync(1)
    ...
    response = cirrus_syncing_node.blockstore.get_last_balance_update_transaction(request_model=request_model)
    assert isinstance(response, GetLastBalanceUpdateTransactionModel)
