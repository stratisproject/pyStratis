...
class uint64(customint):
    ...
    def __radd__(self, other) -> uint64:
        if other == 0:
            return self
        else:
            return self.__add__(other)

    def __sub__(self, other) -> uint64:
        if isinstance(other, customint):
            return uint64(self.value - other.value)
        if isinstance(other, int):
            return uint64(self.value - other)
    ...
    def __rsub__(self, other) -> uint64:
        if other == 0:
            return self
        else:
            return self.__sub__(other)

    def __mul__(self, other) -> uint64:
        if isinstance(other, customint):
            return uint64(self.value * other.value)
        if isinstance(other, int):
            return uint64(self.value * other)
    ...
    def __imul__(self, other) -> uint64:
        if other == 0:
            return uint64(0)
        else:
            return self.__mul__(other)

    def __rmul__(self, other) -> uint64:
        if other == 0:
            return uint64(0)
        else:
            return self.__mul__(other)

    def __divmod__(self, other) -> Tuple[uint64, uint64]:
        if isinstance(other, customint):
            if other.value == 0:
                raise ZeroDivisionError('Cannot divide by zero.')
            new_quot, new_mod = self.value // other.value, self.value % other.value
            return uint64(new_quot), uint64(new_mod)
        if isinstance(other, int):
            if other == 0:
                raise ZeroDivisionError('Cannot divide by zero.')
    ...
    def __rdivmod__(self, other) -> Tuple[uint64, uint64]:
        return self.__divmod__(other)

    def __floordiv__(self, other) -> uint64:
        if isinstance(other, customint):
            if other.value == 0:
                raise ZeroDivisionError('Cannot divide by zero.')
            return uint64(self.value // other.value)
    ...
    def __pow__(self, power, modulo=None) -> uint64:
        if isinstance(power, int):
            return uint64(self.value ** power)
